...
interface ISubjectForm extends IAreaSampleSubjectPayload {
  whichSubject: string;
  useSamplingPlan: string;
  samplingPlan: string | null;
}

export function AreaSampleSubjectCreatePage() {
  ...
  const [data, setData] = useState<
    Partial<ISubjectForm & IAreaSampleSubjectPayload>
  >({
    whichSubject: 'area',
  ...
  const {
    ...
    formStatus,
    hasFormErrors,
  } = useForm<ISubjectForm, IAreaSampleSubject>({
    schema,
    uischema: uiSchema,
  ...
  const {
    submissionStatus: areaSubmissionStatus,
    submitHandler: areaSubmitHandler,
    submissionError: areaSubmissionError,
  } = useSubmitHandler({
    isNewRecord: true,
    data: { ...data, surveyMoment: surveyMoment, chemicals: false },
    recordId: '',
    submitSuccessHandler,
    useCreateMutation: useCreateAreaSampleSubjectMutation,
  });
  ...
  useEffect(() => {
    if (samplingPlanData && data.useSamplingPlan === 'samplingPlan') {
      setData(
        produce((draft) => {
          draft.taskDescription = samplingPlanData.taskDescription;
        }),
      );
    }
  ...
  return (
    <>
      {areaSubmissionStatus === SUBMISSION_STATUS.FAILED && (
        <TransitionsModal
          title="An error occurred on the server"
          description="Please try to submit the form again"
          errorList={
            areaSubmissionError?.data as unknown as {
              [key: string]: [string];
            }
          }
          showButton
      ...
      {areaSubmissionStatus === SUBMISSION_STATUS.SAVING && (
        <TransitionsModal
